...
| PACK1 | Packed pages draw the same pictures | `--polish-preview --capture --capture-1x` trace identical with and without `assets/packed/`; sampled frames differ only at cropped-rectangle filter edges (≤12 summed RGB levels, <1% of pixels). `pack.rs` tests: alpha bounds, `refit`, shelf placement without overlap, manifest round trip. Startup log prints `packed: N cells on M pages` and `assets loaded in X ms`. |
| SEL1 | One selector table, one style table | `sprites::SELECTORS` is the only order in which authored families are tried; `sprites::STYLES` is the only source of rigid / pinned / trail / cut rules. `style_table_reproduces_the_reviewed_rules`, `every_cell_has_a_manifest_key_that_parses_back`; all prior anim/sequence tests pass unchanged. |
...
## First animation review milestone — final audit

ART67 completes jab material; all69 implemented moves and16 Action variants have recorded reviewed coverage. Final audit cross-checks source definitions,all65 referenced atlas files,the eight benchmark excerpts,unchanged sim/frame-data tree,185 passing tests/clippy/release and current integration/smoke/control evidence. The final local review package has13 labeled selections,accepted before/after footage,diagnostics,build instructions,residual polish and stick questions. Older bulk capture folders were retired by user-authorized fleet cleanup; the historical acceptance ledger and canonical sources remain. Physical stick and competitive acceptance follow. See the animation review,coverage,reference and audit documents.

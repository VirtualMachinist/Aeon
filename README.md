...
Airborne comparison: `cargo run --release -p aeon -- --kit-preview --kit-air --kit-move=AirShot`. Thirty-two gun cases cover hop/full jump, hit/standing/low guard/miss, both facings at center/corners. `--kit-jump=hop|full`, move and response filters apply before `--kit-case`. The six air normals in the wider 224-case fixture remain awaiting visual review.
